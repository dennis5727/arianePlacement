...
# --- added for the comparison study (ariane hard-macro RL on Kaggle) ---
...
class PPO():
    ...
    def __init__(self):
        ...
        try:
            ...
            # Kaggle without Internet can't download the weights; fall back to
            # random init (trains, just slightly slower to converge).
            print("resnet18 pretrained download failed ({}); using random init".format(e))
...
def main():
    ...
    # --- best-HPWL tracking (added for the comparison study) -------------- #
    outdir = args.outdir or "."
    os.makedirs(outdir, exist_ok=True)
    fig_dir = os.path.join(outdir, "figures")
    os.makedirs(fig_dir, exist_ok=True)
    best_hpwl = float("inf")
    best_hpwl_path = os.path.join(
        outdir, "best_hpwl_{}_pnm{}_seed{}.csv".format(benchmark, placed_num_macro, args.seed))
    best_hpwl_writer = open(best_hpwl_path, "w")
    best_hpwl_writer.write("episode,hpwl,cost,running_reward\n")
    best_hpwl_writer.flush()
    print("best-HPWL log -> {}".format(best_hpwl_path))
    # --------------------------------------------------------------------- #

    log_dir = os.path.join(outdir, "logs")
    os.makedirs(log_dir, exist_ok=True)
    log_file_name = os.path.join(
        log_dir, "log_" + benchmark + "_" + strftime + "_seed_" + str(args.seed) + "_pnm_" + str(args.pnm) + ".csv")
    ...
    for i_epoch in range(args.max_episodes):
        score = 0
        raw_score = 0
        start = time.time()
        state = env.reset()
        ...
        # --- best-HPWL tracking: comp_res is the same metric the greedy/LLM
        #     study reports, so this number is directly comparable. ---------- #
        try:
            hpwl, cost = comp_res(placedb, env.node_pos, env.ratio)
        except Exception as e:
            print("comp_res failed at epoch {}: {}".format(i_epoch, e))
            hpwl, cost = float("inf"), float("inf")
        if hpwl < best_hpwl:
            best_hpwl = hpwl
            best_hpwl_writer.write("{},{:.4f},{:.4f},{:.2f}\n".format(i_epoch, hpwl, cost, running_reward))
            best_hpwl_writer.flush()
            print("*** NEW BEST HPWL = {:.2f}  (cost {:.2f})  epoch {} ***".format(hpwl, cost, i_epoch))
            # overwrite a single best-model + best-placement so disk stays small
            torch.save({"actor_net_dict": agent.actor_net.state_dict(),
                        "critic_net_dict": agent.critic_net.state_dict()},
                       os.path.join(outdir, "best_model_{}_pnm{}.pkl".format(benchmark, placed_num_macro)))
            save_placement(os.path.join(outdir, "best_{}_pnm{}.pl".format(benchmark, placed_num_macro)),
                           env.node_pos, env.ratio)
            try:
                env.save_fig(os.path.join(fig_dir, "best_{}_hpwl{}.png".format(benchmark, int(hpwl))))
            except Exception:
                pass
        # --------------------------------------------------------------------- #
